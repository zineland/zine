...
//
// The regular expression can be used to match the CSS image url:
//
// (?i) - This is an "inline" regular expression option that makes the regular expression case-insensitive. This means that it will match "url(" as well as "URL(".
// url\( - Matches the characters "url(" literally (case-insensitively, due to the (?i) option)
// '? - Matches either a single quote, or none at all.
static CSS_STATIC_URL_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r#"(?i)url\('?/static.*?'?\)"#).unwrap());
static CSS_ROOT_URL_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r#"(?i)url\('?/.*?'?\)"#).unwrap());
                ...
                element!("body>div.bg-primary.text-main", |el| {
                    if let Some(style) = el.get_attribute("style") {
                        ...
                        let backgrond_image_url = match pairs.get("background-image") {
                            Some(value) if CSS_STATIC_URL_REGEX.is_match(value) => {
                            ...
                            Some(value) if CSS_ROOT_URL_REGEX.is_match(value) => {
...
mod tests {
    ...
    #[test_case(r#"<body><div class="bg-primary text-main" style="background-image: url('/test.png');"></div></body>"#; "lowercase")]
    #[test_case(r#"<body><div class="bg-primary text-main" style="background-image: URL('/test.png');"></div></body>"#; "uppercase")]
    ...
    #[test_case(r#"<body><div class="bg-primary text-main" style="background-image: url('/static/test.png');"></div></body>"#; "lowercase")]
